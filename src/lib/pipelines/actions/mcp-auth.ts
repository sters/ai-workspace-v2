...
export function buildMcpAuthPhase(
  serverName: string,
  forceReauth: boolean,
  ...
  return {
    ...
    fn: async (ctx) => {
      const { emitStatus, emitTerminal, signal } = ctx;
      ...
      try {
        ...
        emitStatus(`[debug] forceReauth=${forceReauth}, serverIndex=${serverIndex}`);
        ...
        emitStatus(`[debug] Sending ENTER to select "${forceReauth ? "Re-authenticate" : "Authenticate"}"`);
        proc.terminal.write("\r");
        await delay(300);
        output = await collectOutput(listeners, 1000, 10000);
