...
- **Model resolution** — 6-tier priority in `src/lib/config/model.ts`: explicit model > per-operation per-step config > per-operation config > global operations model > code-level `STEP_DEFAULT_MODELS` > CLI default. Plan/execute/review-shaped steps (`analyze-readme`, `plan-todo`, `execute`, `code-review`, `verify-readme`, …) default to opus; `create-pr`, `autonomous-gate`, `readme-clarity-gate`, `best-of-n-reviewer`, `suggest-workspace` to sonnet; `verify-todo`, `collect-reviews`, `deep-search`, `aggregate-suggestions` to haiku.
- **Effort resolution** — `resolveEffort()` in the same file mirrors the model chain exactly (explicit > per-step config > per-type config > global `operations.effort` > `STEP_DEFAULT_EFFORTS` > CLI default) and is passed to the CLI as `--effort`. `STEP_DEFAULT_EFFORTS` only lists steps that should deviate from the CLI default: `execute` → `xhigh` (hardest long-horizon coding), `create-pr` / `best-of-n-reviewer` / `readme-clarity-gate` / `suggest-workspace` / `prune-suggestions` → `medium`, `collect-reviews` / `verify-todo` / `deep-search` / `aggregate-suggestions` → `low`. Decision-critical steps whose mistakes cost a whole pipeline cycle (`autonomous-gate`, `code-review`) are deliberately left on the default rather than downgraded. Effort — not the model tier — is the primary cost/latency dial; both are wired through `phase-runners.ts` / `context-builder.ts` for single phases, group children, `runChild`, and `runChildGroup`.
