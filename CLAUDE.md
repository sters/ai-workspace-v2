...
`operations` accepts `model` and `effort` both globally and per operation type, and per step via `operations.<operation-type>.steps.<step-type>.{model,effort}`. `effort` takes any CLI level (`low` / `medium` / `high` / `xhigh` / `max`); `null` falls through to the code-level per-step default (see **Effort resolution** in Architecture). The generated `config.yml` documents the built-in per-step model and effort tables in a comment block that `migrateConfigContent` keeps in sync — `TYPE_OVERRIDE_HINT_LINES` in `src/lib/config/migration.ts` is the single source of that block for both generation and migration, so update it there when the defaults change.
...
- **Model / effort resolution** — `resolveModel()` and `resolveEffort()` in `src/lib/config/model.ts` share one 6-tier priority chain: explicit (phase/child) > per-operation per-step config > per-operation config > global `operations.model` / `operations.effort` > code-level `STEP_DEFAULT_MODELS` / `STEP_DEFAULT_EFFORTS` > CLI default. Both are wired through `phase-runners.ts` / `context-builder.ts` for single phases, group children, `runChild`, and `runChildGroup` (plus `suggest-workspace.ts`, which spawns outside the pipeline). **Effort, not the model tier, is the primary cost/latency dial** — hence effort is declared explicitly for every step that declares a model rather than inherited from the CLI. The two tables must cover exactly the same step types; `effort.test.ts` fails on drift.
  ...
  | model | steps | effort |
  |---|---|---|
  | opus | `analyze-readme`, `plan-todo`, `execute`, `research`, `verify-readme`, `code-review`, `coordinate-todos`, `update-readme`, `best-of-n-reviewer` | high |
  | opus | `plan-todo-from-review`, `discover-constraints`, `review-todos`, `update-todo` | medium |
  | sonnet | `autonomous-gate` | high |
  | sonnet | `create-pr`, `best-of-n-file-reviewer`, `best-of-n-synthesizer`, `readme-clarity-gate`, `suggest-workspace`, `prune-suggestions` | medium |
  | sonnet | `collect-reviews`, `verify-todo`, `deep-search`, `aggregate-suggestions` | low |

  Both tables cover **every** `STEP_TYPES` value, so a new step type forces a tier choice. There is **no haiku tier**: a current-generation Sonnet at low effort beats a smaller model at high effort for comparable spend on these steps, and the extraction steps feed the autonomous gate where a silent misread is expensive. Effort tiers track the *shape* of the work, not its importance — `high` for open-ended investigation or a judgment the pipeline then enforces as fact (the README contract, the loop decision), `medium` for bounded translation over an input that already says what to do, `low` for extraction over already-structured text. `xhigh` / `max` are deliberately never code-level defaults: reach for them per-workload via config, measured.

  Two placements are non-obvious. `research` is one step type covering all three research phases (per-repo + cross-repo findings, recommendations, integration), tiered for the findings phases since they are the operation's deliverable rather than a step toward one. The best-of-N steps are split by *what they write*, not by their role in the pipeline: `best-of-n-reviewer` (code candidates, `best-of-n.ts`) both picks a winner and, on `synthesize`, merges the candidates' implementations into the original worktree — hence opus/high, matching `execute`. `best-of-n-file-reviewer` + `best-of-n-synthesizer` (markdown candidates, `best-of-n-files.ts`) only pick and splice documents. Step type names track their system-prompt file names (`best-of-n-reviewer` / `best-of-n-file-reviewer`).
