...
- **Slack bot** — Optional Socket Mode bot in `bin/slack-server.ts` (started by `bin/start.ts` when `slack.enabled` and both tokens resolve non-empty). Listens for `app_mention` events; mentions from non-allowlisted Slack user IDs (`slack.allowedUserIds`) are silently dropped. The `init` command triggers an operation: `init <description>` POSTs to `/api/operations/autonomous` with `startWith: "init"` (full autonomous loop), and `init --only <description>` POSTs to `/api/operations/init` (single init operation, no chain). **Any mention text that is not `init`/`help` is treated as free-form conversation** (`parseCommand` returns `kind: "chat"`) and answered by a read-only Claude query (`converse()` in `src/lib/slack-server/conversation.ts`). Both force `interactionLevel: "low"` since Slack is kick-off only — progress and any AskUserQuestion follow-ups are handled in the WebUI. On accept, the bot posts "OK! I'll start your request!" once in the mention thread; dispatch failures post "Sorry, failed to start: …". **Completion notification (autonomous only)**: when `init` (without `--only`) finishes, a polling notifier (`src/lib/slack-server/notifier.ts`, 30 s interval) posts back to the original thread. The mapping is persisted in the `slack_pending_notifications` SQLite table (migration v7) so it survives slack-server restarts. The notifier scans the operation's events for the `Create PR` phase only (label hardcoded in `src/lib/pipelines/autonomous.ts`), runs `extractPrUrls()` (`src/lib/workspace/pr-url.ts`) over those events' data, and subtracts any URL that already appeared in the operation's input description (so URLs the user pasted as references aren't reported as "created"). Replies with either `Done! Created PRs: • <url> …` or `Done! No PRs were created. Please check details on WebUI.`. Failed runs are silently dropped (the row is deleted but no message posted) — users check the WebUI for details. `init --only` is not tracked since it never produces PRs. **Thread link**: when the mention is posted inside an existing thread, the bot calls `chat.getPermalink` for the thread root and appends the resulting URL to the description via `mergeWithThreadLink()` in `src/lib/slack-server/thread-link.ts` (separator `--- Slack thread ---`). The thread body is intentionally NOT inlined — folding the entire thread (CI bot output, alerts, tangential replies) tended to drown out the actual request and confuse downstream planning, so init / plan operations are expected to fetch the thread on demand if they need the context. Empty descriptions are accepted in that case — the permalink alone becomes the description. If `chat.getPermalink` fails (channel not accessible, network), the failure is logged and the typed description is used as-is. Parser is `src/lib/slack-server/commands.ts`; HTTP layer is `src/lib/slack-server/dispatcher.ts`. **Conversation** (`src/lib/slack-server/conversation.ts`): `converse(threadKey, message)` runs `runClaude` in-process (not via the HTTP API) at the workspace root with `skipAskUserQuestion` and **no `allowedTools`/`addDirs`** — it inherits the host's ambient Claude permissions (the same `~/.claude/settings.json` every other operation runs under), so Bash/git/gh/MCP work wherever the environment already permits them. Read-only discipline is **not** enforced at the tool layer — it's enforced entirely by a strong system prompt (`getSlackChatSystemPrompt()` in `src/lib/templates/prompts/slack-chat.ts`, applied via `appendSystemPromptFile: ensureGlobalSystemPrompt("slack-chat")`), so keep its "DO NOT CHANGE ANYTHING" language strong. MCP servers resolve from the workspace-root `.mcp.json` / user scope (cwd is the root, so per-repo `.mcp.json` is not picked up). Continuity is per Slack thread — the CLI `session_id` keyed on `thread_ts ?? ts` (TTL 2 h, cap 200) is persisted in the `slack_conversation_sessions` SQLite table (migration v8) via `src/lib/db/slack-sessions.ts`, so follow-ups `--resume` the same session **and threads survive a slack-server restart**. To support this, `RunClaudeOptions.resumeSessionId` seeds the initial spawn and `ClaudeProcess.getSessionId()` exposes the captured id (`src/lib/claude/cli.ts`). If a resumed turn fails with no output / non-zero exit (e.g. the persisted session was pruned by the CLI), `converse()` drops the stored id and retries **once** as a fresh conversation. Model/effort come from `config.slack.chatModel` / `config.slack.chatEffort` (defaults sonnet / medium). The handler (`handleConversation` in `index.ts`) posts a "🤔 Thinking…" placeholder, then `chat.update`s it with the reply (falling back to a fresh `chat.postMessage` on any Slack API error). Prompt builder: `buildSlackChatPrompt()` in `src/lib/templates/prompts/slack-chat.ts` (full instructions on the first turn, bare message on resume turns). **Per-user memory** (`slack.memoryEnabled`, default `true`): each Slack user gets a persistent memory the conversation can recall/persist across threads. It lives in a **separate** SQLite file `{workspaceRoot}/.ai-workspace/slack-memory.sqlite` (schema ensured by `ensureSlackMemoryDb()` in `src/lib/slack-server/memory-db.ts`, NOT part of the main `db.sqlite` migrations) so a stray model query can never touch operational data. There is no TS read/write path and no separate extraction call — the conversation Claude operates the DB **itself** via the `sqlite3` CLI, guided by instructions `buildSlackChatPrompt` folds into the first turn (DB path, `memories(id, user_id, content, …)` schema, the user's id, and scoped SELECT/INSERT examples). Memory is scoped strictly by Slack `user_id` (no shared/global memory); the model is told to only ever touch the current user's rows. This is the **one** write allowed in the otherwise read-only session: `getSlackChatSystemPrompt()` carries a "MEMORY EXCEPTION" carve-out permitting writes ONLY to that `memories` table and ONLY when the user explicitly asks to remember something — keep that language precise. `userId` flows from `mention.user` → `handleConversation` → `converse(threadKey, message, { userId, threadContext })`; when `userId` is missing or `memoryEnabled` is false, the memory block is omitted and the session is purely read-only. **Thread folding**: when the *first* conversation mention lands inside an existing thread (`mention.thread_ts` set and `hasThreadSession()` false), `fetchThreadTranscript()` (`src/lib/slack-server/thread-fetch.ts`) pulls the whole thread via `conversations.replies` (paginated, cap 50 pages × 200) and folds it into the first-turn prompt so the user can ask "summarize this thread". The triggering mention is excluded to avoid duplication, and the fetch happens before the placeholder is posted so it isn't included. Resume turns skip the fetch (the CLI session already holds the context). Requires a Slack history scope for the channel type (`channels:history` / `groups:history` / `im:history` / `mpim:history`); on any failure the conversation proceeds without thread context.
